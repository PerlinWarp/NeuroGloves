...
class Classifier(object):
	...
	def train(self, X, Y):
		...
		if self.X.shape[0] >= K * SUBSAMPLE:
			...
			print("Training shape: " ,self.X[::SUBSAMPLE].shape)
		else:
			self.nn = None
	...
	def classify(self, d):
		if (self.X.shape[0] < K * SUBSAMPLE):
			# Not enough samples, just predict 0 
			return 0
		d = np.array(d).reshape(-1,1)
		print("d", d)
		preds = self.nn.predict(d)
		pred = int(preds[0])
		print(pred)
		return pred
...
class MyoClassifier(Myo):
	...
	def __init__(self, cls, tty=None):
		Myo.__init__(self, tty, raw = False, filtered=True)
		self.cls = cls
		...
		while True:
			...
			for i in range(10):
				...
				if m.cls.nn is not None:
					emg_inp = np.array(hnd.emg).reshape((-1, 8)) 
					dists, inds = m.cls.nn.kneighbors(emg_inp)
